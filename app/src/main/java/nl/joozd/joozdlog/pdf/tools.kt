...
import nl.joozd.joozdlog.data.miscClasses.Address
import nl.joozd.joozdlog.data.miscClasses.TotalsForward
...
object pdfTools {
    ...
    fun drawSecondPage(canvas: Canvas, address: Address) {
    ...
    fun fillLeftPage(canvas: Canvas, flights: List<Flight>, totalsForward: TotalsForward){
        ...
        flights.forEach{f ->
            if (!f.sim) {
                ...
                canvas.drawText(
                    if (f.actualAircraft != null) f.actualAircraft!!.typeString else f.aircraft,
                ...
                if (f.actualAircraft?.multipilot ?: 0 > 0) {      // if aircraft is known and MP
                    canvas.drawText(
    ...
    fun fillRightPage(canvas: Canvas, flights: List<Flight>, signatures: Map<Int, String>, totalsForward: TotalsForward) {
        ...
        flights.forEach { f ->
            if (f.sim) {
                ...
                if (f.flightID in signatures.keys){
                    Log.d(TAG, "flightID: ${f.flightID}")
                    val svg = SVG.getFromString(signatures[f.flightID])
